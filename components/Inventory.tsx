...
    // Asegurar que el totalAmount se resetee o se mantenga 0
...
          newTemp[newIndex] = prevTemp[oldIndex] || String(item.quantity).replace(".",",");
        });
        return newTemp;
      });
      
...
      // Corregido el acceso al array, asegurando que existe antes de actualizar
...
  // 🛑 CORRECCIÓN: Convertir a async y usar await en onBulkUpdateInventoryItems
...
    // 🛑 CORRECCIÓN: Usar Promise.all si onSavePurchaseOrder es asíncrona, o mantener forEach si es síncrona
    purchaseOrders
      .filter((o) => o.status === PurchaseOrderStatus.Completed)
      .forEach((order) => {
        onSavePurchaseOrder({
...
            !currentPurchaseOrder.items.some((oi, i) => i !== index && oi.inventoryItemId === item.id)
          );
